...
$area = $_SESSION['area'];
$eficiencia = $_SESSION['eficiencia'];
$placas = $_SESSION["paneles"];
$graficas = array();
...
function generacion($radiacion, $area, $eficiencia){
...
$consulta = "SELECT * FROM radiacion_solar WHERE Fecha BETWEEN '$minDate' AND '$maxDate'";
$lectura = mysqli_query($conn, $consulta);
while($lecturas = $lectura->fetch_array()){
  //$fechas[] = $lecturas["Fecha"];
  $generacion[] = $placas * generacion($lecturas["Radiacion"], $area, $eficiencia);
}
  ...
  if(in_array('generacion', $graficas)){
    ...
    if(count($generacion) > 50 && count($generacion) > 50){
      ...
      for($i = 0; $i < 50; $i++){
        ...
        if($i < 49){$fechas_resumen[$i] = $fechas[$i*$paso_con];}
        else{$fechas_resumen[$i] = $fechas[($i*$paso_con)+$paso_con-1];}
    ...
    elseif(count($generacion) < 50){
      ...
      for($i = 0; $i < count($generacion); $i++){
        ...
        if($i < 49){$fechas_resumen[$i] = $fechas[$i*$paso];}
        else{$fechas_resumen[$i] = $fechas[($i*$paso)+$paso-1];}
    ...
    if(count($generacion) > 50){
      ...
      for($i = 0; $i < 50; $i++){
        ...
        for($c=0; $c < $paso_gen; $c++){
          $generacion_resumen[$i] += $generacion[($i*$paso_gen)+$c];
          $fechas_resumen[$i] = $fechas[($i*$paso_gen)+$c];
        }
      }
    }
    elseif(count($generacion) < 50){
