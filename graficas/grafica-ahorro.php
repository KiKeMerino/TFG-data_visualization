...
$consulta = "SELECT SUM(Radiacion) AS 'radiacion_media' FROM radiacion_solar WHERE YEAR(Fecha) >= $year GROUP BY YEAR(Fecha), MONTH(Fecha) order by Fecha Asc limit 12;";
$lectura = mysqli_query($conn, $consulta);
while($lecturas = $lectura->fetch_array()){
  $generacion[] = $placas * generacion($lecturas["radiacion_media"], $area, $eficiencia);
}
...
var myLineChart = new Chart(ctx, {
  ...
  data: {
    ...
    datasets: [{
      label: "Ahorro",
      backgroundColor: "rgba(46, 30, 250, 0.6)",
